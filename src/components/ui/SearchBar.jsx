  ...
  useEffect(() => {
    ...
    if (
      ...
      dispatch(fetchCategories({ tree: true, active: true, maxDepth: 3 }))
        .then((action) => {
          if (ignore) return;
          const nextCategories = getCategoryListFromResponse(
            action?.payload?.data,
      ...
      <div className="rounded-full border border-[#1B1D604D] bg-white shadow-sm outline-0 transition-all duration-200">
        <div className="flex h-[46px] w-full items-center overflow-visible rounded-full border-none bg-white pl-0 pr-0 outline-none">
          ...
          <input
            ...
            className="h-full  text-base w-full flex-1 border-none font-medium bg-transparent pl-5 pr-4 sm:px-4 text-[var(--customer-ink)] outline-none ring-0 placeholder:text-[#2E2E2E] focus:ring-0 focus-visible:outline-none"
          />
