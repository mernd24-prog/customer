...
export default function CartSummary({
  ...
  const shipping = items.reduce(
    (acc, item) => acc + item.shipping * item.quantity,
    0,
  );
    ...
    >
      ...
      <div className="mt-5 space-y-4 sm:mt-6">
        <div className="flex items-center justify-between gap-3  text-xs sm:text-sm">
          <span className="text-[var(--customer-muted)]">
            Items ({items.length})
          </span>
        ...
        <div className="flex items-center justify-between gap-3  text-xs sm:text-sm">
          <div className="flex items-center gap-1">
            ...
            {showInfoIcon && (
              <button
                type="button"
                className="text-[var(--customer-subtle)] transition-all duration-300 ease-in-out hover:text-[var(--customer-navy)]"
      ...
      <div className="flex items-center justify-between gap-3 pt-1">
        <span className=" text-sm font-bold text-[var(--customer-ink)] sm:text-base md:text-lg">
          Total
        </span>
        <span className=" text-sm font-bold text-[var(--customer-navy)] sm:text-base md:text-lg">
          {fmt(total)}
        </span>
      </div>
      ...
      <div className="mt-4 text-center  text-xs leading-5 text-[var(--customer-muted)] sm:text-sm">
