...
export default function FooterActionLinks({ items = [] }) {
  ...
  return (
    <FooterSectionContainer className="grid grid-cols-1 gap-3 border-y border-white/10 py-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
      {items.map((item, index) => (
        ...
        >
          <img className="h-9 w-9 object-contain" src={item?.icon} alt={item?.alt || item?.label || "Action"} />
          {item?.label}
        </FooterLink>
      ))}
