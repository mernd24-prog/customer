...
export function ShowMoreText({
  ...
  useEffect(() => {
    ...
    const measureOverflow = () => {
      ...
      const maxHeight = singleLineHeight * safeLineLimit + (singleLineHeight * 0.2);
    ...
    if (typeof window.ResizeObserver !== "undefined" && lineTextRef.current && lineTextRef.current.parentElement) {
  ...
  const displayText =
    expanded || !isTruncated
      ? showMore.text
      : `${finalPreview}${ellipsis}`;

  return (
    <span className={className}>
      <span ref={isLineMode ? lineTextRef : null} className={textClassName}>{displayText}</span>
      {isTruncated && (
        <button
          type="button"
          onClick={(e) => {
