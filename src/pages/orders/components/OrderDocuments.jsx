...
export default function OrderDocuments({
  ...
  const docCount = (downloadableDocuments?.length || 0) + (visiblePendingSellerDocuments?.length || 0);

  return (
    <section className="rounded-[16px] border border-[#CE9F2D66] bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-bold text-[#1B1D60] flex items-center gap-2">
            <FileText size={18} className="text-[#CE9F2D]" />
            {selectedOrderItem
              ? "Selected Item Documents"
              : "Order Documents"}
          </h2>
          <p className="mt-1 text-xs font-medium text-[#5F6078]">
            {selectedOrderItem
