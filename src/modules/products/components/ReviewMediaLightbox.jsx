      ...
      >
        <div className="flex items-center justify-between border-b border-[#CE9F2D33] bg-[#FAFAFA]/60 px-4 py-2.5">
          <p className="text-sm font-bold text-black">
            Review Photos{" "}
            {showNavigation ? `(${safeIndex + 1}/${images.length})` : ""}
          </p>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="relative flex min-h-[220px] items-center justify-center bg-[#FAFAFA] p-3 sm:min-h-[320px] sm:p-4">
          {showNavigation && (
            <button
              ...
              className="absolute left-2 top-1/2 z-10 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-full border border-[#CE9F2D66] bg-white text-[#1B1D60] shadow-sm transition hover:bg-[#CE9F2D] hover:text-white sm:left-10"
          ...
          <img
            ...
            className="max-h-[44vh] max-w-[82%] rounded-lg object-contain"
          />
          {showNavigation && (
            <button
              type="button"
              onClick={() => onIndexChange((safeIndex + 1) % images.length)}
              className="absolute right-2 top-1/2 z-10 grid h-8 w-8 -translate-y-1/2 place-items-center rounded-full border border-[#CE9F2D66] bg-white text-[#1B1D60] shadow-sm transition hover:bg-[#CE9F2D] hover:text-white sm:right-10"
          ...
          <div className="flex justify-center gap-2 overflow-x-auto border-t border-[#CE9F2D33] bg-white p-2.5">
            {images.map((image, thumbIndex) => (
              <button
                ...
                className={`h-16 w-16 shrink-0 overflow-hidden rounded-md border bg-white p-0.5 transition ${thumbIndex === safeIndex ? "border-[#CE9F2D]" : "border-[#E8DFC9] opacity-70 hover:border-[#CE9F2D] hover:opacity-100"}`}
              >
                <img
                  loading="lazy"
                  width="400"
