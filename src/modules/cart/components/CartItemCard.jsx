...
export default function CartItemCard({
  ...
  const productPath = getProductPublicPath(
    item?.product || item?.productId || item,
  );
  ...
  return (
    ...
    >
      <div
        className={`grid grid-cols-1 sm:grid-cols-[170px_1fr] lg:grid-cols-[190px_1fr] gap-5 sm:gap-6 pb-6 sm:pb-7 ${
        ...
        <div className="min-w-0 flex flex-col justify-between py-1">
          <div>
            ...
            {productPath ? (
              <Link
                to={productPath}
                className="block mb-2.5 font-bold text-sm sm:text-base text-[#2d2d2d] transition hover:text-[#1B1D60]"
              >
                <ShowMoreText
                  text={item?.title}
                  mode="characters"
              ...
              <h3 className="line-clamp-2 block mb-2.5 font-bold text-sm sm:text-base text-[#2d2d2d]">
                {item?.title}
              </h3>
            )}
